...
def calcular_fluxo_comercial(ano, paises, agrupado):
    ...
    # A validação agora retorna os países inválidos
    ...
    # Carregamento dos dataframes com cache
    ...
    if any(df is None or df.empty for df in
           [df_exp_ano, df_exp_ano_anterior, df_imp_ano, df_imp_ano_anterior, df_ncm, df_exp_mun, df_imp_mun,
            df_uf_mun]):
        st.error("Erro ao carregar um ou mais arquivos CSV. Verifique o ano e tente novamente.")
        return None
    ...
    return (balanca_ano, balanca_ano_anterior, fluxo_comercial_ano, fluxo_comercial_ano_anterior, variacao_balanca, variacao_fluxo,
            ...
            posicao_pais_mg_imp, participacao_pais_mg_imp, ano_completo, ultimo_mes_disponivel, produtos_exportacao, exportacao_pais_ano,
            produtos_importacao, importacao_pais_ano, df_ncm, exportacoes_por_municipio, total_exportacoes_municipios,
...
class DocumentoApp:
    ...
    def finalizar_documento(self):
        ...
        # --- ALTERAÇÃO AQUI: Salva os bytes brutos ---
        ...
        # --- ALTERAÇÃO AQUI: Retorna os bytes, não o stream ---
...
if st.button(" Iniciar Geração do Relatório"):
    ...
    # --- Limpa o state antes de gerar novos arquivos ---
    st.session_state.arquivos_gerados = []
    # --- FIM ---

    # --- Definição automática do caminho do logo ---
    ...
    with st.spinner(f"Gerando relatório para {paises_input} ({ano_selecionado})... Isso pode levar alguns minutos."):
        ...
        try:
            if agrupado:
                ...
                st.info("Iniciando análise agrupada...")
                app = DocumentoApp(logo_path=logo_path_to_use, diretorio_base=diretorio_ui)

                resultados = calcular_fluxo_comercial(ano_selecionado, paises, agrupado=True)
                ...
                st.info("Iniciando análise separada para cada país...")

                for pais in paises:
                    st.subheader(f"Processando: {pais}")
                    app = DocumentoApp(logo_path=logo_path_to_use, diretorio_base=diretorio_ui)

                    resultados = calcular_fluxo_comercial(ano_selecionado, [pais], agrupado=False)
...
if st.session_state.arquivos_gerados:
    ...
    # --- ALTERAÇÃO AQUI: Lógica de ZIP para múltiplos arquivos ---
